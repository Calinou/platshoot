...
@rpc("any_peer", "call_local") func builtin_notice(type: int) -> void:
	match type:
		BuiltinNoticeType.PLAYER_DEATH:
			if get_tree().get_multiplayer().get_remote_sender_id() == get_tree().get_multiplayer().get_unique_id():
				notice("You died.")
			else:
